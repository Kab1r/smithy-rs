...
/**
 * End-to-end regression test for CRITICAL_03: generated deserializers must bound
 * shape-tree recursion so that a deeply-nested payload returns a deserialization
 * error rather than overflowing the thread stack.
 *
 * The model below mirrors `.kiro/CRITICAL_03_codegen-deserializer-recursion-depth/poc.smithy`:
 * a `TreeService` with a `PutTree` operation whose input is the recursive
 ...
 *    exhausted.
 *  - Large enough that the *patched* code comfortably returns `Err(...)` with
