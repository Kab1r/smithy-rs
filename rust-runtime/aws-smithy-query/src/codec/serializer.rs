...
enum CollectionContext {
    List { index: usize },
    Map { index: usize, expecting_key: bool },
}
            ...
            if let Some(ctx) = self.context_stack.last_mut() {
                match ctx {
                    ...
                    CollectionContext::Map {
                        ...
                        if *expecting_key {
                            write!(self.output, "{}.{}.key", self.prefix, index).unwrap();
                            *expecting_key = false;
                        } else {
                            write!(self.output, "{}.{}.value", self.prefix, index).unwrap();
                            *expecting_key = true;
                            *index += 1;
                        }
                    }
...
impl ShapeSerializer for QueryShapeSerializer {
    ...
    fn write_list(
        ...
        let flat = schema.xml_flattened().is_some();
        self.push_prefix(self.wire_name(schema));
        if !flat {
            self.push_prefix("member");
        }
        self.context_stack
